...
<header>
  ...
  <div class="subtitle" id="legend">Score 0 to 10 expresses live exploitability. Protected rows are reported with the reason, never offered for removal.</div>
</header>
...
<section class="selection-bar">
  ...
  <span>Copies the native command for each selected row. Read them before you run them. Nothing runs from this page.</span>
</section>
  ...
  tbody.innerHTML = filtered.map((r, idx) => {
    ...
    return `
      <tr class="${isAccepted ? 'accepted' : ''}${isSelected ? ' selected' : ''}" data-id="${esc(r.Id)}">
        <td class="select-cell"><input type="checkbox" class="select-cb" data-id="${esc(r.Id)}"${r.Protected ? ' disabled title="Protected: this one is not offered for removal"' : ''}${isSelected ? ' checked' : ''}></td>
        <td>${scoreHtml}</td>
