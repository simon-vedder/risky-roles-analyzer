...
2. `src/__ModuleName__/__ModuleName__.psd1`: set `ModuleVersion`; drop `Prerelease` for a stable
...
6. Check the Gallery listing (`Find-Module __ModuleName__ -AllowPrerelease`) and the GitHub release.
<!-- automation-only -->
   `deploy/main.bicep` users point `moduleVersion` at the new version; the module link resolves to
   the Gallery package by default.
<!-- /automation-only -->
